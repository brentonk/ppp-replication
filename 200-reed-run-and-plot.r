...
dat <- as.tbl(read.dta("data-reed-et-al-2008.dta"))
...
points <- as.tbl(read.dta("data-idealpoint4600.dta")) %>%
  mutate(A_ccode = sprintf("%003d", ccode1),
         B_ccode = sprintf("%003d", ccode2),
         udyear = paste(A_ccode, B_ccode, year, sep = "_"),
...
foo <- as.tbl(read.csv("data-nmc-4.0.csv")) %>%
...
tikz(file = "results/latex/fig-rcnw-gull.tex",
     width = 5.4,
     height = 4.5)
ggplot(plot_data,
       aes(x = p, y = pp)) +
