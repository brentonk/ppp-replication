...
data_sai_2006 <- read.csv("data-sobek-abouharb-ingram-2006.csv")
doe_dyad <- read.csv("results/predict-dyad.csv")

## rename the splines that start with an underscore
names(data_sai_2006)[names(data_sai_2006) == "X_spline1mid"] <- "spline1mid"
names(data_sai_2006)[names(data_sai_2006) == "X_spline2mid"] <- "spline2mid"
names(data_sai_2006)[names(data_sai_2006) == "X_spline3mid"] <- "spline3mid"
...
save(cr_sai_2006,
     doe_sai_2006,
     file = "results/replications/sobek-abouharb-ingram-2006.rda")
