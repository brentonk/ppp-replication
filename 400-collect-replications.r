...
replication_files <- list.files(path = "results/replications",
                                pattern = "\\.rda$",
                                full.names = TRUE)
replication_table <- foreach (x = replication_files, .combine = "rbind") %do% {
    ...
    fit_name <- sub("^results/replications/", "", x)
...
replication_info <- yaml_to_df("metadata-replications.yml")
...
print(
    ...
    file = "results/latex/tab-replications.tex"
)
...
print(
    ...
    include.rownames = FALSE,
    sanitize.text.function = identity,
    file = "results/latex/tab-replications-appendix.tex"
)
